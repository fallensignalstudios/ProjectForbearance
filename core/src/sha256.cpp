...
std::string sha256_hex(const std::string& bytes) {
  ...
  for (std::size_t i = 0; i < full; ++i) compress(h, data + i * 64);

  std::vector<unsigned char> tail(data + full * 64, data + len);
  tail.push_back(0x80);
  while (tail.size() % 64 != 56) tail.push_back(0);
  std::uint64_t bits = static_cast<std::uint64_t>(len) * 8ULL;
  for (int i = 7; i >= 0; --i) tail.push_back(static_cast<unsigned char>((bits >> (i * 8)) & 0xFF));
  for (std::size_t i = 0; i < tail.size(); i += 64) compress(h, tail.data() + i);
